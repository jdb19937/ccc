 ...
 */
static int
verifica_relationes(const char *nomen, const Polyhedron *ph,
                    int p, int q)
{
    int errores = 0;
    ...
    if (p * ph->facies != 2 * ph->latera) {
        printf("  %s: p*F = %d, 2*E = %d — NON CONCORDANT\n",
    ...
    if (q * ph->vertices != 2 * ph->latera) {
        printf("  %s: q*V = %d, 2*E = %d — NON CONCORDANT\n",
...
main(void)
{
    ...
    for (int i = 0; i < NUMEROSITAS_SOLIDORUM; i++) {
        ...
        printf("  Dualis %s: V=%d E=%d F=%d, chi=%d\n",
