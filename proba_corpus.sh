...
printf "%-16s" "PLICA"
printf " %7s" "CC/CC" "CCC/LDI" "CC/LDI" "CCC/CC"
printf " "
printf " %7s" "CC/CC" "CCC/LDI" "CC/LDI" "CCC/CC"
printf "\n"
...
for fons in "$DIR"/*.c; do
    ...
    bin_ccc_cc="$TMP/${nomen}_ccc_cc"
    out_cc="$TMP/${nomen}_cc.out"
    out_ccc="$TMP/${nomen}_ccc.out"
    out_cc_ldi="$TMP/${nomen}_cc_ldi.out"
    out_ccc_cc="$TMP/${nomen}_ccc_cc.out"

    # 1. cc/cc
    obj_cc_cc="$TMP/${nomen}_cc_cc.o"
    $CC -std=c99 -O0 -c -o "$obj_cc_cc" "$fons" 2>/dev/null
    cc_comp=$?
    if [ $cc_comp -eq 0 ]; then
        $CC -o "$bin_cc" "$obj_cc_cc" 2>/dev/null
    ...
    $CCC -Scapita -c -o "$obj_ccc" "$fons" >/dev/null 2>&1
    ccc_cc_comp=$?
    if [ $ccc_cc_comp -eq 0 ]; then
        $CC -o "$bin_ccc_cc" "$obj_ccc" 2>/dev/null
        if [ $? -ne 0 ]; then ccc_cc_comp="link"; fi
    else ccc_cc_comp="comp"; fi

    cc_run="-"; ccc_run="-"; cc_ldi_run="-"; ccc_cc_run="-"
    cmp1="-"; cmp2="-"; cmp3="-"
    ...
    if [ "$ccc_cc_comp" = "0" ]; then
        comp_ccc_cc_ok=$((comp_ccc_cc_ok + 1))
        curre_cum_limite "$bin_ccc_cc" > "$out_ccc_cc" 2>&1
        ccc_cc_run=$?
        if [ "$cc_run" != "-" ] && [ "$ccc_cc_run" -eq 0 ] && [ "$cc_run" -eq 0 ]; then
            if diff -q "$out_cc" "$out_ccc_cc" >/dev/null 2>&1; then
                ccc_cc_run="par"; par_ccc_cc=$((par_ccc_cc + 1))
            else ccc_cc_run="dis"; fi
        fi
    fi
    ...
    has_bad=0
    for v in "$ccc_comp" "$cc_ldi_comp" "$ccc_cc_comp"; do
        case "$v" in comp|link) has_bad=1 ;; esac
    done
    for v in "$ccc_run" "$cc_ldi_run" "$ccc_cc_run"; do
    ...
    for v in "$cc_comp" "$ccc_comp" "$cc_ldi_comp" "$ccc_cc_comp"; do
        case "$v" in comp|link) printf " ${R}%7s${Z}" "$v" ;; *) printf " %7s" "$v" ;; esac
    done
    printf " "
    for v in "$cc_run" "$ccc_run" "$cc_ldi_run" "$ccc_cc_run"; do
        case "$v" in
            par) printf " ${G}%7s${Z}" "$v" ;;
            dis) printf " ${R}%7s${Z}" "$v" ;;
            -|0) printf " %7s" "$v" ;;
...
echo "ccc/cc compilavit:       $comp_ccc_cc_ok / $tot"
echo "par cc/cc vs ccc/ldi:    $par / $tot"
echo "par cc/cc vs cc/ldi:     $par_cc_ldi / $tot"
echo "par cc/cc vs ccc/cc:     $par_ccc_cc / $tot"
